...
class Report extends React.Component {
    ...
    handleSearch = (e) => {
        ...
        api.get(`/reports?fromDate=${fromDate}&toDate=${toDate}`)
    ...
    loadData = () => {
        ...
        return this.state.locations.map(({ longitude, latitude, elevation, dateTime, Product }) => {
            return (
                <tr>
                    <td>{Product.id}</td>
    ...
    exportExcel = () => {
        ...
        return (
            <div className="row text-center" style={{ marginTop: '100px', display: "inline" }}>
                <Workbook filename={filename} element={<button className="ui blue button" style={{ position: "absolute", right: "0", marginRight: "1%" }}>Export</button>}>
                    <Workbook.Sheet data={this.state.locations} name="Export Data">
                        <Workbook.Column label="ProductID" value={location => location.Product.id} />
                        <Workbook.Column label="Desciption" value={location => location.Product.description} />
    ...
    render() {
        return (
            <React.Fragment>
                ...
                <div class="field">
                    <div className="ui huge header">Report
                    {this.exportExcel()}
                    </div>
                </div>
