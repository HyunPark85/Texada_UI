...
class ProductDetail extends React.Component {
  ...
  componentDidMount() {
    api.get(`products/${this.props.match.params.id}`).then(response => {
      this.setState({ product: response.data });
    });
    api
      .get(`products/${this.props.match.params.id}/locations`)
      .then(response => {
  ...
  loadDetail = () => {
    return this.state.productInfo.map(
      ({ longitude, latitude, elevation, dateTime, id, ProductId }) => {
        return (
          <tr>
            <Location
